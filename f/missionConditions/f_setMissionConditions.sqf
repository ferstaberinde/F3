...
if (isServer) then {
	_weatherParams = [f_param_weather, f_param_fog == 4, f_param_wind == 4] call f_fnc_setWeather;
	...
	_fogParams = [f_param_fog] call f_fnc_setFog;
	if (!isNil "_fogParams") then {
		f_var_conditions_params set [6,_fogParams];
	};

	_windParams = [f_param_wind] call f_fnc_setWind;
	if (!isNil "_windParams") then {
		f_var_conditions_params set [5,_windParams];
	};

	_dateParams = [f_param_timeOfDay] call f_fnc_setTime;
